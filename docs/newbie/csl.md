...
如果出现了上述情况，请参考本文中的 [手动修改配置文件](#edit-csl-config) 部分进行操作，以使其优先从 HelloSkin 加载材质。
...
CustomSkinLoader 14.4 起支持通过 ExtraList 的方式添加皮肤站，这也是我们推荐的方法之一。你可以在用户中心的「皮肤 Mod」页面下载到 HelloSkin 的 ExtraList 文件，将其放入 `.minecraft/CustomSkinLoader/ExtraList/` 目录下即可。
...
::: tip 提示
HelloSkin 仅支持 `1.7.10-v10a` 或更高版本的 SkinPort。
...
client {
    ...
    S:hostCustomServer2Cape=https://helloskin.cn/cape/%name%.png  // [!code focus]
    S:hostCustomServer2Skin=https://helloskin.cn/skin/%name%.png  // [!code focus]
    B:useCrafatar=false
...
![默认配置 且 Mojang 上没有同名角色时的加载流程](./assets/mods/flow-regular.png)
:::

::: details 2. 默认配置 & Mojang 上有同名角色
此时若要优先从 HelloSkin 中加载皮肤，可参照本文中的 [手动修改配置文件](#edit-csl-config) 部分进行操作，以达到 **3.** 的效果。

![默认配置 且 Mojang 上有同名角色时的加载流程](./assets/mods/flow-mojang-only.png)
:::

::: details 3. 手动修改配置文件，将 HelloSkin 设置为最高优先级
若参照本文中的 [手动修改配置文件](#edit-csl-config) 部分进行操作，即可达到以下效果。

![修改了配置文件，将 HelloSkin 设置为最高优先级时的加载流程](./assets/mods/flow-helloskin-only.png)
:::

::: details 4. 默认配置 & Mojang 上有同名角色 & 尝试加载全部材质
若 Mojang 中的同名角色设置了皮肤但未设置披风，CustomSkinLoader 会尝试从后续的服务器加载披风。

![默认配置，尝试加载全部材质时的加载流程](./assets/mods/flow-loadall.png)
:::
