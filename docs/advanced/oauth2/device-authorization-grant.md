...
>
> HelloSkin 运营组可通过请求 ID 快速了解请求详情，因此，在通过 HelloSkin 官方渠道寻求协助时，请务必提供请求 ID。

## 申请设备代码流白名单

由于公共客户端存在潜在的安全问题，若要使用设备代码流获取访问令牌，需要先为应用申请设备代码流白名单。

请先在  [<BSSection>OAuth 2 应用管理</BSSection>](https://helloskin.cn/user/oauth/manage) 中，将应用的回调 URL 设置为 `https://helloskin.cn/oauth/callback`，然后使用你的 HelloSkin 账号绑定的邮箱发送 [邮件工单](../../email.md)，在邮件标题中注明「申请 OAuth 设备代码流白名单」，并在邮件正文中提供你的应用名称和客户端 ID。如应用需要申请 Yggdrasil 相关权限，还应在邮件正文中提供具体需要申请的 Yggdrasil 相关权限的列表。

HelloSkin 运营组会在一周内审核你的申请，视情况将应用添加至白名单中，并通过邮件回复审核结果。如果运营组长时间未回复，请加入我们的 [官方用户交流群](../../user-group.md)，联系站点管理员处理。
...
客户端需要向 HelloSkin 发送如下请求，请求一个设备代码对：

```http
// 请求体中的换行只是为了方便阅读，实际发起请求时不应换行，下同
POST https://helloskin.cn/oauth/device_code HTTP/1.1
...
> - 用户授权完成后，HelloSkin 会在 ID 令牌中添加 `selectedProfile` 声明，其值即为用户选择的角色的 Yggdrasil 档案。
>
> ⚠️ 该特性暂时只在设备代码流中支持。对于授权代码流，需要在应用内实现角色选择。

❌ 如果应用不在白名单内，HelloSkin 会返回 `invalid_client` 错误；  
✅ 如果应用在白名单内，HelloSkin 将返回如下响应：
...

{
    ...
    "verification_uri": "https://helloskin.cn/oauth/link",
    "verification_uri_complete": "https://helloskin.cn/oauth/link?user_code={{user_code}}",
    "expires_in": 300,
    "interval": 5
}
```
...
在等待用户完成授权时，应用应以 `interval` 为间隔，向 HelloSkin 发送如下请求，轮询授权结果：
...
如用户尚未完成授权，HelloSkin 将返回 `authorization_pending` 错误；用户完成授权后，HelloSkin 会返回如下响应：
...
ID 令牌是一个 JWT，其无法作为访问令牌使用，但可用于识别用户身份。当前 HelloSkin 将 ID 令牌作为传递用户身份信息的一种方式，直接对 ID 令牌进行 JWT 验证和解析即可提取出其中包含的用户身份信息：
...
> [!NOTE] 注意
> 当前 HelloSkin 仅在设备代码流中返回 ID 令牌。对于授权代码流，请通过 [Blessing Skin API](../api#blessing-skin-api-文档) 请求用户信息。

#### 获取用于验证 ID 令牌的密钥

HelloSkin 部分实现了 OpenID Connect Discovery。ID 令牌中的 `iss` 声明的值为 JWT 签发者的标识符，是一个 URL。应用可以通过拼接 URL `{iss}/.well-known/openid-configuration` 并向其发起 GET 请求来获取 OpenID Connect Discovery 元数据。

在 OpenID Connect Discovery 元数据中，有一项 `jwks_uri` 属性，其值为一个 URL。向该 URL 发起 GET 请求即可获取到一个 JWKS，其中包含的 JWK 即可用于验证 HelloSkin 签发的 ID 令牌的签名。

> [!WARNING] ID 令牌的签名算法
> 除 RS256 外，HelloSkin 还可能会使用 PS256、ES256 和 EdDSA 算法对 ID 令牌进行签名。在验证 ID 令牌之前，请确定你使用的 JWT 库支持这些算法。
